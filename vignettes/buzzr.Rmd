...
vignette: >
  %\VignetteIndexEntry{Using buzzr to handle buzzdetect data}
...
- **return_ident**: available in `read_directory` and `bin_directory`, should the file identifier be placed into its own column?
The ident is the relative path from the results directory to the file, stripped of its extension and the `_buzzdetect` tag.
...
This isn't terribly useful if we're dealing with a single file,
