...
rebin <- function(dt_bin, binwidth){
  ...
  has_time_common <- 'time_common' %in% names(dt_bin)
  if(has_time_common){dt_bin$time_common <- NULL}
  ...
  if(has_time_common){dt_rebin$time_common <- commontime(dt_rebin$start_bin)}
  return(dt_rebin)
}
